...
class ModeloTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            let modelsTobeDeleted = modelosOfTableView[indexPath.row]
            self.modelosOfTableView.remove(at: indexPath.row)
            tableView.deleteRows(at: [indexPath], with: .fade)
            DatabaseController.persistentContainer.viewContext.delete(modelsTobeDeleted)
            DatabaseController.saveContext()
        }
    }
