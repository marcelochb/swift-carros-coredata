...
class MarcasTableViewController: UITableViewController,UIPickerViewDelegate, UIPickerViewDataSource
{
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            let marcaTobeDeleted = marcasOfTableView[indexPath.row]
            self.marcasOfTableView.remove(at: indexPath.row)
            tableView.deleteRows(at: [indexPath], with: .fade)
            DatabaseController.persistentContainer.viewContext.delete(marcaTobeDeleted)
            DatabaseController.saveContext()
        }
    }
